...
- (void)setupWithAudioController:(AEAudioController *)audioController {
    ...
    if ( !AECheckOSStatus(result=AUGraphAddNode(_audioGraph, &_componentDescription, &_node), "AUGraphAddNode") ||
         ...
         !AECheckOSStatus(result=AUGraphNodeInfo(_audioGraph, _node, NULL, &_audioUnit), "AUGraphNodeInfo2")) {
    ...
    result = AudioUnitGetProperty(_audioUnit, kAudioUnitProperty_FactoryPresets, kAudioUnitScope_Global, 0, &mEQPresetsArray, &size);
...
//    AUPreset *aPreset = (AUPreset*)CFArrayGetValueAtIndex(mEQPresetsArray, 2);
//    OSStatus result1 = AudioUnitSetProperty(_audioUnit, kAudioUnitProperty_PresentPreset, kAudioUnitScope_Global, 0, aPreset, sizeof(AUPreset));
//    if (result1) { printf("AudioUnitSetProperty result %ld %08X %4.4s\n", result1, (unsigned int)result1, (char*)&result1); return; };
//    
//    printf("SET EQ PRESET %d ", 2);
