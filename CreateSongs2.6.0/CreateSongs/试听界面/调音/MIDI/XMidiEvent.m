...
//    if ([TYCommonClass sharedTYCommonClass].songType == womanSong) {
    self.noteMessage.note = self.noteMessage.note + 12;
//    }
    
    [XOpenALPlayer playSound:self.noteMessage height:self.height];
}
...
- (void)playSoundByNote:(XMidiNoteMessage *)message byInstrumentType:(int)type {
    ...
    [XOpenALPlayer playSoundByNotePitch:message withInstrumType:type];
}
