...
#define SLIDER_EDG UIEdgeInsetsMake(-1, 14, 1, 20)
...
- (void) setUpperValue:(float)upperValue
{
    ...
    [self.delegate volumeIsChanged:_upperValue];
...
//        if(IS_PRE_IOS7())
//        {
//            UIImage* image = [self imageFromBundle:@"调节按钮"];
//            _lowerHandleImageNormal = [image imageWithAlignmentRectInsets:SLIDER_EDG];
//        }
//        else
//        {
//            UIImage *image = [self imageFromBundle:@"调节按钮"];
...
//            _lowerHandleImageHighlighted = [image imageWithAlignmentRectInsets:SLIDER_EDG];
//        }
    }
...
- (UIImage *)upperHandleImageNormal
{
    if(_upperHandleImageNormal==nil)
    {
        ...
        {
            UIImage *image = [self imageFromBundle:@"slider-default7-handle"];
            _upperHandleImageNormal = [image imageWithAlignmentRectInsets:SLIDER_EDG];
        }
    }
...
- (UIImage *)upperHandleImageHighlighted
{
    if(_upperHandleImageHighlighted==nil)
    {
        ...
        {
            UIImage *image = [self imageFromBundle:@"slider-default7-handle"];
            _upperHandleImageHighlighted = [image imageWithAlignmentRectInsets:SLIDER_EDG];
        }
    }
...
//    self.track = [[UIImageView alloc] init];
    self.track.backgroundColor = [UIColor colorWithHexString:@"#451d11"];
...
//    self.trackBackground = [[UIImageView alloc] init];
    self.trackBackground.frame = [self trackBackgroundRect];
    self.trackBackground.backgroundColor = [UIColor colorWithHexString:@"#451d11"];
    
    self.rightImage = [UIImageView new];
    self.rightImage.image = [UIImage imageNamed:@"调节右边椭圆"];
...
-(void)layoutSubviews
{
    ...
    CGRect rect1 = [self trackBackgroundRect];
    CGRect rect2 = [self trackRect];
...
//    rect1 = CGRectMake(rect2.origin.x, rect2.origin.y, rect2.size.width + rect1.size.width, 0.5);
    
    
    
    self.trackBackground.frame = rect1;
    self.track.frame = rect2;
    
    self.trackBackground.centerY = self.height / 2;
    self.track.centerY = self.height / 2;
    
    self.rightImage.frame = CGRectMake(rect1.origin.x + rect1.size.width-4.5, 0, 4.5, 4.5);
    self.rightImage.centerY = self.height / 2;
    
//    NSLog(@"%@-%@", NSStringFromCGRect(self.trackBackground.frame), NSStringFromCGRect(self.track.frame));
