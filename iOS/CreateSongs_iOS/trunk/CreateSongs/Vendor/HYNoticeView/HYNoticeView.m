...
#define TRIANGLE 5.0
#define NoticeColor HexStringColor(@"#ffdc74")
...
-(void)drawTriangle:(CGContextRef)ctx andRect:(CGRect)rect{
    ...
    switch (_position) {
        case HYNoticeViewPositionTop:
            sPoints[0] =CGPointMake(rect.size.width*0.75, 0);//坐标1
            sPoints[1] =CGPointMake(rect.size.width * 0.75 - SIDELENGTH * 0.5, TRIANGLE);//坐标2
            sPoints[2] =CGPointMake(rect.size.width * 0.75 + SIDELENGTH * 0.5, TRIANGLE);//坐标3
            break;
