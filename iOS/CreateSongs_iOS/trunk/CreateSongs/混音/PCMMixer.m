...
static
inline
BOOL removeMix_buffers(const int16_t *buffer1,
...
//    NSLog(@"+++%d", mixbufferNumSamples);
	...
	for (int i = 0 ; i < mixbufferNumSamples; i++) {
		int32_t s1 = buffer1[i];
		int32_t s2 = buffer2[i];
		int32_t mixed = s1 * app.personVolume + s2 * app.banzouVolume;
...
+ (OSStatus) mix:(NSString*)file1 file2:(NSString*)file2 offset:(int)offset mixfile:(NSString*)mixfile isMixer:(BOOL)isMixer
{
...
//        NSLog(@"%d--%d", numPackets1, numPackets2);
            ...
            BOOL clipping;
            if (isMixer) {
                clipping = mix_buffers((const int16_t *)buffer1, (const int16_t *)buffer2,
                                       (int16_t *) mixbuffer, numSamples);
            } else {
                clipping = removeMix_buffers((const int16_t *)buffer1, (const int16_t *)buffer2, (int16_t *) mixbuffer, numSamples);
            }
...
+ (OSStatus)removeMixFiles:(NSArray *)files atTimes:(NSArray *)times toMixfile:(NSString *)mixfile {
    ...
    for (int i=1; i<[files count]; i++) {
        ...
        status = [self mix:file1 file2:file2 offset:[(NSNumber*)[times objectAtIndex:i] intValue]*5 mixfile:target isMixer:NO];
    }
    return status;
}
...
+ (OSStatus) mixFiles:(NSArray*)files atTimes:(NSArray*)times toMixfile:(NSString*)mixfile {
  ...
  for (int i=1; i<[files count]; i++) {
    ...
    status = [self mix:file1 file2:file2 offset:[(NSNumber*)[times objectAtIndex:i] intValue]*5 mixfile:target isMixer:YES];
  }
  return status;
}
