...
{
    self = [self initWithFrame:frame horizontalPadding:16 verticalPadding:10 inputViewMinHeight:36 inputViewMaxHeight:150 type:EMChatToolbarTypeGroup];
    if (self) {
...
{
    self = [self initWithFrame:frame horizontalPadding:16 verticalPadding:10 inputViewMinHeight:36 inputViewMaxHeight:150 type:type];
    if (self) {
...
- (void)_setupSubviews
{
    ...
    _toolbarBackgroundImageView.backgroundColor = HexStringColor(@"#eeeeee");
...
//    NSEaseLocalizedString(@"message.toolBar.inputPlaceHolder", @"input a new message");
...
//    [self.moreButton setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_keyboard"] forState:UIControlStateSelected];
    self.moreButton.titleLabel.font = JIACU_FONT(12);
    [self.moreButton setTitle:@"发送" forState:UIControlStateNormal];
    [self.moreButton setTitleColor:HexStringColor(@"#441D11") forState:UIControlStateNormal];
//    [self.moreButton setTitleColor:HexStringColor(@"#ffffff") forState:UIControlStateHighlighted];
//    self.moreButton.backgroundColor = HexStringColor(@"#ffffff");
    [self.moreButton setBackgroundImage:[UIImage imageNamed:@"白色背景"] forState:UIControlStateNormal];
...
- (void)setInputViewRightItems:(NSArray *)inputViewRightItems
{
    ...
    CGFloat rightPadding = 17;
    
//    CGFloat oMaxX = self.toolbarView.frame.size.width - self.horizontalPadding;
    CGFloat oMaxX = self.toolbarView.frame.size.width - rightPadding;
    CGFloat itemHeight = self.toolbarView.frame.size.height - self.verticalPadding * 2;
