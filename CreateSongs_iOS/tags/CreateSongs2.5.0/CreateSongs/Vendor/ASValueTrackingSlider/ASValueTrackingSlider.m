...
- (UIImage *)thumbImageForState:(UIControlState)state {
    UIImage *thumbImage = [super thumbImageForState:state];
    thumbImage = [thumbImage imageWithAlignmentRectInsets:UIEdgeInsetsMake(-1, 20, -1, 20)];
    return thumbImage;
}
...
- (CGRect)trackRectForBounds:(CGRect)bounds {
    return CGRectMake(0, (bounds.size.height-1.5)/2, self.frame.size.width, 1.5);
}
...
{
    if (self.dragThumbBlock) {
        self.dragThumbBlock(self.value);
    }
