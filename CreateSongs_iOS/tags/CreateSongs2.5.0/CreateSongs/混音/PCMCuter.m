...
static
inline
BOOL removeCutMix_buffers(const int16_t *buffer1,
...
static
inline
BOOL cutMix_buffers(const int16_t *buffer1,
                 const int16_t *buffer2,
                 int16_t *mixbuffer,
                 int mixbufferNumSamples)
{
    //    NSLog(@"+++%d", mixbufferNumSamples);
    BOOL clipping = FALSE;
    ...
    for (int i = 0 ; i < mixbufferNumSamples; i++) {
        int32_t s1 = buffer1[i];
        int32_t s2 = buffer2[i];
        int32_t mixed = s1 * app.personVolume ;
...
@implementation PCMCuter

+ (void) cut_setDefaultAudioFormatFlags:(AudioStreamBasicDescription*)audioFormatPtr
...
+ (OSStatus) cutMix:(NSString*)file1 file2:(NSString*)file2 offset:(int)offset mixfile:(NSString*)mixfile isMixer:(BOOL)isMixer
{
...
#define BUFFER_SIZE 1024
    ...
    [self cut_setDefaultAudioFormatFlags:&inputDataFormat numChannels:2];
    ...
    while (TRUE) {
        ...
        // 这里条件判断 当其中一个是0的时候就中止合成 也就是以最短的时长为准
        ...
        //        NSLog(@"%d--%d", numPackets1, numPackets2);
        ...
        BOOL clipping;
        if (isMixer) {
            clipping = cutMix_buffers((const int16_t *)buffer1, (const int16_t *)buffer2,
                                   (int16_t *) mixbuffer, numSamples);
        } else {
            clipping = removeCutMix_buffers((const int16_t *)buffer1, (const int16_t *)buffer2, (int16_t *) mixbuffer, numSamples);
        }
...
+ (OSStatus)removeMixFiles:(NSArray *)files atTimes:(NSArray *)times toMixfile:(NSString *)mixfile {
    ...
    for (int i=1; i<[files count]; i++) {
        ...
        status = [self cutMix:file1 file2:file2 offset:[(NSNumber*)[times objectAtIndex:i] intValue]*5 mixfile:target isMixer:NO];
    }
    return status;
}
...
+ (OSStatus) cutFiles:(NSArray*)files atTimes:(NSArray*)times toMixfile:(NSString*)mixfile {
    ...
    for (int i=1; i<[files count]; i++) {
        ...
        status = [self cutMix:file1 file2:file2 offset:[(NSNumber*)[times objectAtIndex:i] intValue]*5 mixfile:target isMixer:YES];
    }
    return status;
}
